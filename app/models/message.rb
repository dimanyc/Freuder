class Message < ActiveRecord::Base
  ...
  def self.pull_tweets(user)
    ...
    if tweets.each { |tweet| validate_uniqueness_of(user,tweet) }
      tweets.each { |tweet| user.messages.create!(tweet_id: tweet.id, body: tweet.full_text, author: tweet.user.screen_name, author_image_url: tweet.user.profile_image_uri.to_s, mentions: tweet.user_mentions.to_s, hashtags: tweet.hashtags[0].to_s) }
      return true
    else
      return false 
    end
